...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        if (cache_type == IS_LLC)
        {
            ...
            uint32_t remapped_set = start_set + (set % (end_set - start_set + 1));

            set = remapped_set;


            // if (fill_cpu == 0 && set >= set_allocation[0]) {
            //     cout << "invalid set access 0" << endl;
            //     assert(0);
            // } else if (set < set_allocation[0]) {
            //     cout << "invalid set access 1" << endl;
            // }
        } 
