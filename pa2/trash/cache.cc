...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
    {
        ...
        // cout << "cache partition type: " << cache_partition_type << endl;
        // Static Set Partitioning ----------------
        if (cache_type == IS_LLC && cache_partition_type == 2)
        {
            uint32_t sets_per_core = NUM_SET / 2; // Divide sets equally between cores

            // Remap the set to the correct core's assigned range
            if (fill_cpu == 0)
            {
                set = set % sets_per_core; // Map Core 0 sets to range [0, sets_per_core-1]
            }
            else if (fill_cpu == 1)
            {
                set = (set % sets_per_core) + sets_per_core; // Map Core 1 sets to range [sets_per_core, NUM_SET-1]
            }
        }
        ...
        if (cache_type == IS_LLC && cache_partition_type == 3)
        {
            uint32_t start_set = (fill_cpu == 0) ? 0 : set_allocation[0];
            uint32_t end_set = start_set + set_allocation[fill_cpu] - 1;

            // cout << "start set: " << start_set << ", end set: " << end_set << endl;

            // Map original set into CPU’s allocated range
            uint32_t remapped_set = start_set + (set % set_allocation[fill_cpu]);
            set = remapped_set;
        } 
        // ------------------------------
    ...
    for (uint32_t i = 0; i < MAX_READ; i++)
    {
        ...
        if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
        {
            ...
            if (cache_type == IS_LLC && cache_partition_type == 3)
            {
                ...
                uint32_t start_set = (read_cpu == 0) ? 0 : set_allocation[0];
                uint32_t end_set = start_set + set_allocation[read_cpu] - 1;

                // Map original set into CPU’s allocated range
                uint32_t remapped_set = start_set + (set % set_allocation[read_cpu]);
                set = remapped_set;
            }
            // --------------------------------------------------------
            ...
            // Way partitioning assertions
            if (cache_type == IS_LLC && cache_partition_type == 1)
            {
                // cout << "readcpu : " << read_cpu << ", cpu: " << cpu << ", way: " << way << endl;
                if (read_cpu == 0)
                {

                    if (way > 7)
                    {
                        cout << "invalid way access cpu: " << read_cpu << endl;
                        assert(0);
                    }
                }
                else
